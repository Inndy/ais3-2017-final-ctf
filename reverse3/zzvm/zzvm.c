...
int zz_ctx_read_mem(ZZVM_CTX *ctx, ZZ_ADDRESS addr, void *buffer, size_t len)
{
    if(addr + len >= ZZ_MEM_LIMIT) {
        return ZZ_OUT_BOUND;
    }
    memcpy(buffer, ctx->memory + addr, len);
    return ZZ_SUCCESS;
}

int str2int(const char *buff, int len)
{
    int v = 0;
    int i = 0;
    while(i < len && buff[i]) {
...
uint16_t _zz_default_syscall_handler(ZZVM_CTX *ctx)
{
    ...
    switch (ctx->regs.RA) {
        ...
        case 2: // str2int
            zz_ctx_read_mem(ctx, ctx->regs.R1, buff, 8);
            return str2int(buff, 8);
        default:
            zz_msg("[MESSAGE] unknow system call");
            return -1;
    }
    return 0;
}
...
int zz_read_mem(ZZVM *vm, ZZ_ADDRESS addr, void *buffer, size_t len)
{
    ...
    return zz_ctx_read_mem(&vm->ctx, addr, buffer, len);
}
...
{
    snprintf(buffer, limit, "%-8s", ZZ_OP_NAME[ins->op]);
    return ZZ_SUCCESS;
}
...
int _zz_disasm_1r(char *buffer, size_t limit, ZZ_ADDRESS ip, ZZ_INSTRUCTION *ins)
{
    ...
    snprintf(buffer, limit, "%-8s %s", ZZ_OP_NAME[ins->op], ZZ_REGISTER_NAME[r1]);
    return ZZ_SUCCESS;
}
...
int _zz_disasm_2r(char *buffer, size_t limit, ZZ_ADDRESS ip, ZZ_INSTRUCTION *ins)
{
    ...
    snprintf(buffer, limit, "%-8s %s, %s", ZZ_OP_NAME[ins->op], ZZ_REGISTER_NAME[r1],
             ZZ_REGISTER_NAME[r2]);
    return ZZ_SUCCESS;
}
...
int _zz_disasm_3r(char *buffer, size_t limit, ZZ_ADDRESS ip, ZZ_INSTRUCTION *ins)
{
    ...
    snprintf(buffer, limit, "%-8s %s, %s, %s", ZZ_OP_NAME[ins->op],
...
{
    snprintf(buffer, limit, "%-8s 0x%.4x", ZZ_OP_NAME[ins->op], ins->imm);
    return ZZ_SUCCESS;
}
...
int _zz_disasm_1j(char *buffer, size_t limit, ZZ_ADDRESS ip, ZZ_INSTRUCTION *ins)
{
    ...
    snprintf(buffer, limit, "%-8s 0x%.4x", op_name,
...
int _zz_disasm_2i(char *buffer, size_t limit, ZZ_ADDRESS ip, ZZ_INSTRUCTION *ins)
{
    ...
    snprintf(buffer, limit, "%-8s %s, 0x%.4x", ZZ_OP_NAME[ins->op],
...
int _zz_disasm_3i(char *buffer, size_t limit, ZZ_ADDRESS ip, ZZ_INSTRUCTION *ins)
{
    ...
    snprintf(buffer, limit, "%-8s %s, %s, 0x%.4x", ZZ_OP_NAME[ins->op],
             ZZ_REGISTER_NAME[r1], ZZ_REGISTER_NAME[r2],
             ins->imm);
    return ZZ_SUCCESS;
}
